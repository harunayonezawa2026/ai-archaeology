...
### Wikipedia extraction prompt

```
For the ALPAC (Automatic Language Processing Advisory Committee) report
of 1966, extract:
...
### Grading prompt (the strongest)

```
Take the five main recommendations ALPAC made in 1966, and grade each
against the reality of modern LLM translation (the translation quality,
throughput, and cost of GPT-4 / Claude as of 2025). Categorize each as
"right," "wrong," or "neutral."
```

Same pattern as ZISC (post #2) and Token Ring (post #4): **a prompt that grades past claims against modern facts** is the strongest weapon in this series. **Evaluating an old long-form document as "right" or "wrong" deepens understanding of the present by one full step.**
