...
## 7. The Prompts I Used

### Primary-source candidate discovery prompt

```
Search for declassified primary documents on NSA's Project Lightning,
conducted 1957-1962.
Prefer NSA's official declassified PDFs.
Include secondary sources (Wikipedia, industry articles) too.
```

### The 67-year reconciliation prompt (the strongest tool)

```
For each thing NSA Project Lightning (1957) was aiming at —
1 GHz logic circuit, special-purpose machine 100x faster than commercial,
five-company foundational-research collaboration,
state-led semiconductor investment, monopoly on cryptanalysis —
map it to the 2024 reality (NVIDIA H100, CHIPS Act, AI supercomputers,
OpenAI/Anthropic monopoly, superconducting quantum computers)
and judge each as one of:
"achieved 67 years late," "structurally identical," or "revived in different form."
```

The standard prompt of this series is to map a past R&D investment to the present along **four axes: target spec, collaboration structure, monopoly structure, and side-effects**. **Take what was written as "someday we'll do this" half a century ago, and check how it actually got implemented** — that's the strongest tool of Declassified Archaeology.
